...
#define Calibri_18_base 0x000C84
...
static uint8_t __g_font_buf[74]; // 74可以但148不行,不知道为啥
...
static uint8_t *__user_font_getdata(int offset, int size)
{
  ...
  {
    ret = W25Q64_Read_Bytes(__g_font_buf, (Calibri_18_base + offset), size);
    // if (ret != W25Q64_OK)
...
static const uint8_t *__user_font_get_bitmap(const lv_font_t *font, uint32_t unicode_letter)
{
  ...
  if (p_pos[0] != 0)
  {
    ...
    static uint8_t *ret;
    ret = __user_font_getdata(pos + sizeof(glyph_dsc_t), (gdsc->box_w * gdsc->box_h * __g_xbf_hd.bpp) / 8);
