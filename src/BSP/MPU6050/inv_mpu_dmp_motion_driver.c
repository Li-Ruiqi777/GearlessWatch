...
{
    unsigned char fifo_data[MAX_PACKET_LENGTH]; // MAX_PACKET_LENGTH==32
    /* 从FIFO一共取出dmp.packet_length字节数据 = 16(quat)+6(acc)+6(gryo)+4(gesture_decode) */
    unsigned char ii = 0;
    ...
    /* Get a packet. */
    //	__IO int sta = mpu_read_fifo_stream(dmp.packet_length, fifo_data, more);
    if (mpu_read_fifo_stream(dmp.packet_length, fifo_data, more) /*==-2*/)
        return -1; // 这里G了
