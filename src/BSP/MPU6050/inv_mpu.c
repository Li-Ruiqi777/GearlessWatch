...
int mpu_read_fifo_stream(unsigned short length, unsigned char *data, unsigned char *more)
{
  ...
  if (fifo_count > (st.hw->max_fifo >> 1))
  {
    ...
    {
      __IO int8_t sta = mpu_reset_fifo(); /*==0*/
      return -2;
    }
  }

  if (i2c_read(st.hw->addr, st.reg->fifo_r_w, length, data)) // 从FIFO寄存器0x74读取length个数据
    return -1;
...
uint8_t mpu_dmp_init(void)
{
  ...
  if (mpu_init() == 0) // 初始化MPU6050
  {
    ...
    if (res)
      return 2;
    res = mpu_set_sample_rate(100); // 设置采样率10HZ    /DEFAULT_MPU_HZ
    if (res)
      return 3;
...
uint8_t mpu_dmp_get_data(IMU_info *IMU)
{
  ...
  if (sensors & INV_WXYZ_QUAT)
  {
    ...
    q3 = quat[3] / q30;
    // 计算得到俯仰角/横滚角/航向角
    IMU->pitch = asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3;                                    // pitch
    IMU->roll = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 + 1) * 57.3;     // roll
